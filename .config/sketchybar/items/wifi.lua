...
local wifi_up = Sbar.add("item", "wifi.upload", {
  position = "right",
  y_offset = 4,
  padding_left = -5,
  width = 0,
...
local wifi_down = Sbar.add("item", "wifi.download", {
  position = "right",
  y_offset = -4,
  padding_left = -5,
  icon = {
...
wifi:subscribe({ "wifi_change", "system_woke" }, function(_)
  Sbar.exec("ipconfig getifaddr en0", function(ip_)
    local connected = not (ip_ == "")
    wifi:set({
      icon = {
...
local function toggle_details()
  ...
  if should_draw then
    ...
    -- end)
    Sbar.exec("networksetup -listpreferredwirelessnetworks en0 | sed -n '2 p' | tr -d '\t'", function(result)
      ssid:set({ label = result })
    end)
